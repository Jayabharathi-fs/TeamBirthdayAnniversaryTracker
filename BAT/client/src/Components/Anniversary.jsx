...
const columns = [
  ...
  { field: "doj", headerName: "Date of Joining", flex: 1 },
  { field: "daysLeft", headerName: "Days Left", flex: 1 },
];

const paginationModel = { page: 0, pageSize: 5 };

const Anniversary = ({ open, handleClose }) => {
  ...
  const fetchAnniversaries = async (query = "") => {
    try {
      const res = await axios.get(
        "http://localhost:5000/employee/anniversary",
        {
          params: { search: query },
        }
      );
  ...
  useEffect(() => {
    if (open) fetchAnniversaries(searchQuery);
  }, [open, searchQuery]);
